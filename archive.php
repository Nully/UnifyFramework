<?php /* UnifyFramework date template. */ ?>
<?php get_header(); ?>
<div id="main-contents" class="<?php echo uf_get_contents_class(); ?>">
    <h2 id="page-title">
        <?php if(is_day()): ?>
            <?php printf(__("Daily posts : <span>%s</span>", "unify_framework"), get_the_date()); ?>
        <?php elseif(is_month()): ?>
            <?php printf(__("Monthly posts : <span>%s</span>", "unify_framework"), get_the_date("Y F")); ?>
        <?php elseif(is_year()): ?>
